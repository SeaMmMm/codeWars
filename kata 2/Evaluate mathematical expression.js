var calc = function (expresion) {
  expresion = entradauniforme(expresion);

  while (expresion.includes('(')) {
    expresion = detectarParentesis(expresion);
  }
  expresion = entradauniforme(expresion);
...
var entradauniforme = function (expresion) {
  ...
  for (let variable_i = 0; variable_i < expresion.length; variable_i++) {
    if (expresion[variable_i] === '-' && expresion[variable_i + 1] === '-') {
      ...
      expresion[variable_i] === '+' &&
      expresion[variable_i + 1] === '-'
    ) {
      restocadena += '-';
      variable_i++;
    } else {
  ...
  for (let variable_i = 0; variable_i < restocadena.length; variable_i++) {
    if (
      '*/'.includes(restocadena[variable_i]) &&
      '-'.includes(restocadena[variable_i + 1])
    ) {
      if (
        restocadena[variable_i] === '/' &&
        restocadena[variable_i + 1] === '-'
      ) {
  ...
  for (let variable_i = 0; variable_i < Cadena_Split.length; variable_i++) {
    ...
    if (n === '*') {
      ...
      if (
        ...
        Cadena_Split = Cadena_Split.slice(0, indiceMultiplicador[0] + 1).concat(
          Cadena_Split.slice(indiceSiguienteOperacion, Cadena_Split.length)
        );
        break;
  ...
  for (let variable_i = 0; variable_i < Cadena_Split.length; variable_i++) {
    ...
    if (n === '/') {
      ...
      if (
        ...
        Cadena_Split = Cadena_Split.slice(0, indiceDivision[0] + 1).concat(
          Cadena_Split.slice(indiceSiguienteOperacion, Cadena_Split.length)
        );
        break;
      } else if (!Cadena_Split[variable_i + 2]) {
